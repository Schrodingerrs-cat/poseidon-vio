...
class MSCKF(object):
    ...
    def initialize_gravity_and_bias(self):
        ...
        self.state_server.imu_state.acc_bias = sum_linear_acc / len(self.imu_msg_buffer)
        ...
        q0_i_w = self.quaternion_from_two_vectors(gravity_imu, -IMUState.gravity)

        R = to_rotation(q0_i_w)
        R = R.T

        q_final = to_quaternion(R)
        self.state_server.imu_state.orientation = q_final / np.linalg.norm(q_final)
    ...
    def batch_imu_processing(self, time_bound):
        ...
        for imu_msg in self.imu_msg_buffer:
            ...
            m_gyro, m_acc = imu_msg.angular_velocity, imu_msg.linear_acceleration
    ...
    def process_model(self, time, m_gyro, m_acc):
        ...
        F_dt2 = F_dt @ F_dt
        F_dt3 = F_dt2 @ F_dt
        Phi = np.identity(21) + F_dt + F_dt2 / 2 + F_dt3 / 6
        ...
        self.state_server.state_cov = 0.5 * (P + P.T)
    ...
    def predict_new_state(self, dt, gyro, acc):
        ...
        dq_dt , dq_dt2 = np.zeros(4), np.zeros(4)
        if gyro_norm > 1e-5:
            dq_dt = (np.cos(gyro_norm*dt/2)*np.identity(4) + np.sin(gyro_norm*dt/2)/gyro_norm*omega) @ q
            dq_dt2 = (np.cos(gyro_norm*dt/4)*np.identity(4) + np.sin(gyro_norm*dt/4)/gyro_norm*omega) @ q
        else:
            dq_dt = (np.eye(4) + 0.5 * dt * omega) @ q
            dq_dt2 = (np.eye(4) + 0.25 * dt * omega) @ q

        dR_dt_t = dq_dt / np.linalg.norm(dq_dt)
        dR_dt2_t = dq_dt2 / np.linalg.norm(dq_dt2)
        ...
        # Final rotation at dt (already computed)
        # R4_t = dR_dt_transpose
        ...
        v = v + (dt / 6.0) * (k1_v_dot + 2*k2_v_dot + 2*k3_v_dot + k4_v_dot)

        p = p + (dt / 6.0) * (k1_p_dot + 2*k2_p_dot + 2*k3_p_dot + k4_p_dot)
    ...
    def state_augmentation(self, time):
        ...
        camstate = CAMState(self.state_server.imu_state.id)
        self.state_server.cam_states[self.state_server.imu_state.id] = camstate

        camstate.timestamp = time
        camstate.orientation = to_quaternion(R_w_c)
        camstate.position = t_c_w

        camstate.orientation_null = camstate.orientation
        camstate.position_null = camstate.position
        ...
        new_cov[:old_rows, :old_cols] = old_cov
        self.state_server.state_cov = new_cov
        P11 = self.state_server.state_cov[:21, :21]
        P12 = self.state_server.state_cov[:21, 21:]

        # Fill in the augmented state covariance.
        self.state_server.state_cov[old_rows:old_rows+6, 0:old_cols] = np.hstack((J @ P11, J @ P12))
        self.state_server.state_cov[0:old_rows, old_cols:old_cols+6] = self.state_server.state_cov[old_rows:old_rows+6, 0:old_cols].T
