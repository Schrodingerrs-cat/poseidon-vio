...
class MSCKF(object):
    ...
    def initialize_gravity_and_bias(self):
        ...
        # Gyro bias = mean angular velocity when stationary.
        self.state_server.imu_state.gyro_bias = sum_angular_vel / n

        # Gravity in the IMU frame (mean specific-force reads +g when static).
        gravity_imu = sum_linear_acc / n

        # Normalize gravity and save to IMUState.
        gravity_norm = np.linalg.norm(gravity_imu)
        IMUState.gravity = np.array([0., 0., -gravity_norm])

        # Align initial orientation so that R(q) rotates world-frame "up"
        # (-gravity) into the measured gravity_imu direction. With the JPL
        # convention used by to_rotation, this means orientation maps the
        # world frame into the IMU frame.
    ...
    def batch_imu_processing(self, time_bound):
        """
        Process the imu messages in the buffer up to `time_bound`, running
        the process model on each and dropping consumed messages.
        """
        used = 0
        for msg in self.imu_msg_buffer:
            t = msg.timestamp
            if t < self.state_server.imu_state.timestamp:
                used += 1
                continue
            if t > time_bound:
                break
            self.process_model(t, msg.angular_velocity, msg.linear_acceleration)
            used += 1

        # Assign the current IMU state an id, then increment the global counter.
        self.state_server.imu_state.id = IMUState.next_id
        IMUState.next_id += 1

        # Discard consumed IMU messages.
        self.imu_msg_buffer = self.imu_msg_buffer[used:]

    def process_model(self, time, m_gyro, m_acc):
        """
        Section III.A of S-MSCKF, Appendix A of MSCKF.
        Propagates the IMU state and covariance with one bias-corrected IMU
        sample and modifies the transition matrix to enforce the OC-EKF
    ...
    def predict_new_state(self, dt, gyro, acc):
        """
        Propogate the IMU state using 4th order Runge-Kutta.
        Quaternion kinematics use the closed-form Omega propagation (eqn (1)
        in the MSCKF paper); velocity and position use RK4.
        """
        ...
        # Closed-form quaternion propagation for intervals dt and dt/2.
        if gyro_norm > 1e-5:
            ...
            dq_dt = (np.identity(4) + 0.5 * dt * Omega) @ q * np.cos(gyro_norm * dt * 0.5)
            dq_dt2 = (np.identity(4) + 0.25 * dt * Omega) @ q * np.cos(gyro_norm * dt * 0.25)
        ...
        imu_state.orientation = quaternion_normalize(dq_dt)
        imu_state.velocity = v + dt / 6 * (k1_v_dot + 2 * k2_v_dot + 2 * k3_v_dot + k4_v_dot)
        imu_state.position = p + dt / 6 * (k1_p_dot + 2 * k2_p_dot + 2 * k3_p_dot + k4_p_dot)

    
    def state_augmentation(self, time):
        """
        Append a new camera state derived from the current IMU state and
        augment the state covariance using the Jacobian J (MSCKF eqn (16)).
        """
        imu_state = self.state_server.imu_state
        ...
        # Nontrivial block of the augmentation Jacobian (6 x 21).
        J = np.zeros((6, 21))
        J[:3, :3] = R_i_c
        ...
        old_cov = self.state_server.state_cov
        old_size = old_cov.shape[0]
        new_cov = np.zeros((old_size + 6, old_size + 6))
        new_cov[:old_size, :old_size] = old_cov

        P11 = old_cov[:21, :21]
        P1x = old_cov[:21, :]  # shape (21, old_size), includes P12 when cams exist.
        JP = J @ P1x           # shape (6, old_size)

        new_cov[old_size:, :old_size] = JP
        new_cov[:old_size, old_size:] = JP.T
        new_cov[old_size:, old_size:] = J @ P11 @ J.T

        # Symmetrize.
        self.state_server.state_cov = (new_cov + new_cov.T) / 2.0
    ...
    def add_feature_observations(self, feature_msg):
        ...
        for fm in feature_msg.features:
            obs = np.array([fm.u0, fm.v0, fm.u1, fm.v1])
            if fm.id not in self.map_server:
                feat = Feature(fm.id, self.optimization_config)
                feat.observations[state_id] = obs
                self.map_server[fm.id] = feat
            else:
                self.map_server[fm.id].observations[state_id] = obs
                tracked_feature_num += 1

        self.tracking_rate = (tracked_feature_num / curr_feature_num
                              if curr_feature_num > 0 else 0.0)
    ...
    def measurement_update(self, H, r):
        """
        Section III.B: EKF update with QR-compression of a tall H (eqns
        (28)-(29) of S-MSCKF). Applies the correction to IMU + camera states
        with a quaternion error update and updates the covariance.
        """
        if H.shape[0] == 0 or r.shape[0] == 0:
            return

        # Reduce H by thin QR when H is tall, to avoid forming huge matrices.
        ...
        for i, cam_state in enumerate(self.state_server.cam_states.values()):
            dx = delta_x[21 + 6 * i: 21 + 6 * (i + 1)]
            dq_cam = small_angle_quaternion(dx[:3])
            cam_state.orientation = quaternion_multiplication(dq_cam, cam_state.orientation)
            cam_state.position = cam_state.position + dx[3:6]
