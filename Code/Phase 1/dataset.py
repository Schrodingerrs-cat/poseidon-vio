...
class Stereo(object):
    def __init__(self, cam0, cam1):
        # EuRoC sequences occasionally drop a single frame on one camera
        # (e.g. MH_04: 2033 vs 2032). Truncate to the shared length.
        if len(cam0) != len(cam1):
            n = min(len(cam0), len(cam1))
            cam0.ids = cam0.ids[:n]
            cam0.timestamps = cam0.timestamps[:n]
            cam1.ids = cam1.ids[:n]
            cam1.timestamps = cam1.timestamps[:n]
        self.cam0 = cam0
        self.cam1 = cam1
